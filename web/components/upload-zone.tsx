...
function WasmIcon({ state }: { state: UploadState }) {
  ...
  return (
    ...
    >
      <path d="M32 4 L56 18 L56 46 L32 60 L8 46 L8 18 Z" strokeWidth="2"
        stroke={state === 'error' ? '#f87171' : state === 'success' ? '#34d399' : state === 'scanning' ? '#a78bfa' : state === 'hover' ? '#38bdf8' : '#334155'}
        fill={state === 'error' ? 'rgba(248,113,113,0.08)' : state === 'success' ? 'rgba(52,211,153,0.08)' : state === 'scanning' ? 'rgba(167,139,250,0.08)' : state === 'hover' ? 'rgba(56,189,248,0.08)' : 'rgba(30,41,59,0.5)'}
        className="transition-all duration-500"
      />
      <text x="32" y="35" textAnchor="middle" fontSize="11" fontWeight="700" fontFamily="monospace"
        fill={state === 'error' ? '#f87171' : state === 'success' ? '#34d399' : state === 'scanning' ? '#a78bfa' : state === 'hover' ? '#38bdf8' : '#64748b'}
        className="transition-all duration-500"
      >.wasm</text>
    </svg>
  );
...
function ScanningAnimation() {
  return (
    <div className="w-full mt-3 overflow-hidden rounded-full h-1 bg-slate-800">
      <div className="h-full rounded-full bg-gradient-to-r from-violet-500 via-fuchsia-400 to-violet-500"
        style={{ animation: 'scan-sweep 1.6s ease-in-out infinite', backgroundSize: '200% 100%' }}
      />
      <style>{`@keyframes scan-sweep { 0% { transform: translateX(-100%); } 100% { transform: translateX(200%); } }`}</style>
    </div>
  );
}
...
function SpinnerDots() {
  return (
    <div className="flex gap-1.5 items-center justify-center mt-2">
      ...
      <style>{`@keyframes dot-pulse { 0%, 80%, 100% { opacity: 0.2; transform: scale(0.8); } 40% { opacity: 1; transform: scale(1.2); } }`}</style>
    </div>
  );
}
...
function SuccessIcon() {
  return (
    <svg className="w-5 h-5 text-emerald-400 inline-block mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
...
function ErrorIcon() {
  return (
    <svg className="w-5 h-5 text-red-400 inline-block mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
...
export function UploadZone({
    ...
    try {
      ...
      return new Promise((resolve) => {
        reader.onload = async (event) => {
          try {
            ...
            const response = await fetch(backendUrl, {
              ...
              body: JSON.stringify({ wasm_bytes: base64Data, function_name: 'main', args: [] }),
            });
            if (!response.ok) {
              ...
              const wasmError = parseWasmError(response, parsedError);
            ...
            setErrorDetails({ title: 'Validation Error', message: errorMsg, suggestedAction: 'Please try uploading again.' });
            setUploadState('error');
            setDroppedFile(null);
            resolve(false);
          }
        ...
        reader.onerror = () => {
          ...
          setErrorMessage(errorMsg);
          setErrorDetails({ title: 'File Read Error', message: errorMsg, suggestedAction: 'Please try selecting the file again.' });
          setUploadState('error');
          setDroppedFile(null);
          resolve(false);
        };
        try { reader.readAsArrayBuffer(file); } catch (error) {
          const errorMsg = error instanceof Error ? error.message : 'Unable to start reading file';
          setErrorMessage(errorMsg);
          setErrorDetails({ title: 'File Read Error', message: errorMsg, suggestedAction: 'Please try selecting a different file.' });
          setUploadState('error');
          setDroppedFile(null);
          resolve(false);
        }
      ...
      setErrorMessage(errorMsg);
      setErrorDetails({ title: 'Submission Error', message: errorMsg, suggestedAction: 'Please try again.' });
      setUploadState('error');
      setDroppedFile(null);
      return false;
    }
      ...
      reader.onload = (event) => {
        setTimeout(async () => {
          try {
            ...
            if (arrayBuffer.byteLength < 8) throw new Error('File is too small to be a valid WebAssembly module');
            const view = new DataView(arrayBuffer);
            const magicNumber = view.getUint32(0, false);
            if (magicNumber !== 0x0061736d) throw new Error('Invalid WASM magic number. File is not a valid WebAssembly module');
            const version = view.getUint32(4, true);
            if (version !== 1) throw new Error(`Unsupported WASM version: ${version}. Expected version 1`);
  ...
  const displayState = isDragActive && uploadState !== 'scanning' ? 'hover' : uploadState;
  ...
  return (
    <div className="w-full font-sans">
      <div id="wasm-upload-zone" {...getRootProps()}
        className={`relative flex flex-col items-center justify-center border-2 border-dashed rounded-2xl p-10 cursor-pointer transition-all duration-300 ease-in-out select-none min-h-[260px] ${borderColor[displayState] || borderColor.idle} ${bgColor[displayState] || bgColor.idle}`}
        role="button" aria-label="WASM file upload zone"
      >
        ...
        {(displayState === 'hover' || displayState === 'scanning') && (
          <span className="absolute inset-0 rounded-2xl pointer-events-none"
            style={{ boxShadow: displayState === 'hover' ? '0 0 0 1px rgba(56,189,248,0.3)' : '0 0 0 1px rgba(167,139,250,0.35)', animation: 'pulse-ring 2s ease-in-out infinite' }}
          />
        )}
          ...
          <div className="flex flex-col items-center text-center gap-4 transition-all duration-300">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                or{' '}
                <button type="button" className="text-sky-400 underline underline-offset-2 hover:text-sky-300 transition-colors"
        ...
        {uploadState === 'scanning' && (
          <div className="flex flex-col items-center text-center gap-3 w-full px-4">
            ...
            <p className="text-violet-300 font-semibold text-base tracking-wide">Scanning contract…</p>
            {droppedFile && (
        ...
        {uploadState === 'success' && droppedFile && (
          <div className="flex flex-col items-center text-center gap-4">
            ...
            <div>
              <p className="text-emerald-400 font-semibold text-base"><SuccessIcon />Contract uploaded successfully</p>
            ...
            <div className="flex items-center gap-3 bg-slate-800/80 border border-emerald-700/40 rounded-xl px-5 py-3">
              ...
              <div className="text-left">
                <p className="text-sm font-medium text-slate-200 truncate max-w-[220px]">{droppedFile.name}</p>
                <p className="text-xs text-slate-500 font-mono">{formatBytes(droppedFile.sizeBytes)}</p>
              </div>
            </div>
            <button type="button" id="wasm-upload-reset-btn" onClick={handleReset}
        ...
        {uploadState === 'error' && (
          <div className="flex flex-col items-center text-center gap-4">
            ...
            <div>
              <p className="text-red-400 font-semibold text-base"><ErrorIcon />File rejected</p>
              <p className="text-xs text-red-300/70 mt-1 max-w-[280px] leading-relaxed">{errorMessage}</p>
            </div>
            ...
            <button type="button" id="wasm-upload-try-again-btn" onClick={handleReset}
