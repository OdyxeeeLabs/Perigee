# 🔬 Perigee: Soroban Resource Profiler

[![Stellar Wave](https://img.shields.io/badge/Stellar-Wave_Program-blue)](https://www.drips.network/wave/stellar)

**Perigee** is a developer tool designed to provide deep visibility into Soroban smart contract resource consumption (CPU, RAM, and Ledger Footprint).

## 🚀 The Vision

Building on Soroban requires careful resource management. Perigee provides a "Nutrition Label" for your smart contracts, helping you optimize for lower fees and higher performance before you deploy to Mainnet.
...
```bash
git clone https://github.com/SoroLabs/Perigee
cd Perigee
```
...
The **core** crate is a Rust binary that will power Perigee's resource profiling.
...
```bash
# Build the binary
cargo build -p Perigee-core

# Run the server (RUST_LOG=info is required to see API logs)
RUST_LOG=info cargo run -p Perigee-core
```
...
```bash
cargo test -p Perigee-core merkle_tree
```
...
Perigee now supports live simulation via the web dashboard. Connect your wallet, select a function, and get your **Contract Nutrition Label** instantly.
