...
async fn main() {
    ...
    if args.len() > 1 && args[1] == "merkle" {
        if args.len() < 4 {
            eprintln!("Usage: Perigee-core merkle <build|build-file|proof> <args>");
            eprintln!("Commands:");
        ...
        match command.as_str() {
            ...
            "proof" => {
                if args.len() < 5 {
                    eprintln!("Usage: Perigee-core merkle proof <leaf_index> <leaf1> <leaf2> ...");
                    std::process::exit(1);
                }
            ...
            "build-file" => {
                if args.len() < 4 {
                    eprintln!("Usage: Perigee-core merkle build-file <file>");
    ...
    let app = Router::new()
        .route(
            "/",
            get(|| async {
                "Hello from Perigee! Use POST /simulations/analyze to persist + compare simulation metrics."
            }),
        )
