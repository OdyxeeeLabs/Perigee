...
The Perigee Fee Market Prediction System analyzes historical Stellar/Soroban network fee data to provide intelligent recommendations for optimal transaction fee bidding. This ensures your transactions are included in the blockchain within your desired timeframe while minimizing costs.
...
DATABASE_URL=sqlite://Perigee.db    # Database URL for storing fee data
```
...
```bash
cargo test -p Perigee-core fee_analytics
cargo test -p Perigee-core fee_store
cargo test -p Perigee-core fee_collector
```
