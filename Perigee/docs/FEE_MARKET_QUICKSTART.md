...
cd c:\Users\SWAYY\Desktop\skibi\Perigee

# Build the project
cargo build -p Perigee-core

# Run the server
cargo run -p Perigee-core
```
...
```bash
# If using SQLite
sqlite3 Perigee.db "SELECT COUNT(*) FROM ledger_fee_samples;"
```
...
4. **Run tests**: `cargo test -p Perigee-core fee_`
