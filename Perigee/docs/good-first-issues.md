...
- Assert the _content_ of the events match the transaction details.
...
**Context**
Currently, shares are only burned during `withdraw` (which returns assets). This new function should burn shares _without_ returning assets (increasing the value of remaining shares).

**Requirements**

- Implement \`burn(e: Env, from: Address, amount: i128)\`.
- Reduce \`TotalShares\` and user balance.
- Do _not_ send any tokens back to the user.
- Emit \`Burn\` event.
