...
Perigee now provides comprehensive error handling for WASM file uploads and backend validation. When the backend rejects a WASM file, users see detailed, actionable error messages describing what went wrong and how to fix it.
...
```tsx
<UploadZone enableBackendValidation={false} onFileReady={handleFileReady} />
```
...
```bash
RUST_LOG=debug cargo run -p Perigee-core
```
...
**Fix**: Check backend logs with `RUST_LOG=info cargo run -p Perigee-core`
